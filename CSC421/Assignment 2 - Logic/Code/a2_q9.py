# This file contains a lot of code in order for it to be self-contained. 
# You will need to add some code at the end of the source file for your submission 
# There is some code provided to check your output. 
...
# Code for defining Lego world knowledge base
# The code may run longer than the rest of the questions, but it will produce a valid result 
lego_clauses = []
lego_clauses.append(expr("On(x,y) & On(y,z) ==> On(x,z)"))
lego_clauses.append(expr("OnPlate(x) ==> Base(x, x)"))
lego_clauses.append(expr("On(x, z) & Base(z, y) ==> Base(x, y)"))
lego_clauses.append(expr("OnPlate(x) & AtLeft(x, y) ==> Base_at_right(x, y)"))
lego_clauses.append(expr("Base_at_right(x, y) & AtLeft(z, x) ==> Base_at_right(z, y)"))
lego_clauses.append(expr("Base_at_right(x, y) & AtLeft(y, z) ==> Base_at_right(x, z)"))
lego_clauses.append(expr("AtLeft(x, y) ==> Object_at_right(y, x)"))
lego_clauses.append(expr("Object_at_right(y, x) & On(z, x)==> Object_at_right(y, z)"))
lego_clauses.append(expr("Object_at_right(y, x) & On(z, y)==> Object_at_right(z, x)"))

lego_clauses.append(expr("OnPlate(A)"))
lego_clauses.append(expr("OnPlate(D)"))
lego_clauses.append(expr("AtLeft(A, D)"))
lego_clauses.append(expr("On(C, D)"))
lego_clauses.append(expr("On(B, C)"))
lego_clauses.append(expr("OnPlate(F)"))
lego_clauses.append(expr("AtLeft(D, F)"))
lego_clauses.append(expr("On(E, F)"))
lego_clauses.append(expr("OnPlate(J)"))
lego_clauses.append(expr("AtLeft(F, J)"))
lego_clauses.append(expr("On(I, J)"))
lego_clauses.append(expr("On(H, I)"))
lego_clauses.append(expr("On(G, H)"))
lego_clauses.append(expr("Color(A, Red)"))
lego_clauses.append(expr("Type(A, Brick)"))
lego_clauses.append(expr("Color(B, White)"))
lego_clauses.append(expr("Type(B, Tile)"))
lego_clauses.append(expr("Color(C, Brown)"))
lego_clauses.append(expr("Type(C, Plate)"))
lego_clauses.append(expr("Color(D, Grey)"))
lego_clauses.append(expr("Type(D, Brick)"))
lego_clauses.append(expr("Color(E, Brown)"))
lego_clauses.append(expr("Type(E, Tile)"))
lego_clauses.append(expr("Color(F, Red)"))
lego_clauses.append(expr("Type(F, Brick)"))
lego_clauses.append(expr("Color(G, Brown)"))
lego_clauses.append(expr("Type(G, Tile)"))
lego_clauses.append(expr("Color(H, Red)"))
lego_clauses.append(expr("Type(H, Brick)"))
lego_clauses.append(expr("Color(I, Blue)"))
lego_clauses.append(expr("Type(I, Brick)"))
lego_clauses.append(expr("Color(J, Yellow)"))
lego_clauses.append(expr("Type(J, Plate)"))

kb = FolKB(lego_clauses)

if_b_on_c = bool(len(list(fol_fc_ask(kb, expr("On(B, C)"))))) #Is B on top of C

on_c = list(list(fol_fc_ask(kb, expr("On(x, C)")))[0].values())[0] #get the piece that's on top of C
on_c_color_type = list(fol_fc_ask(kb, expr(f"Color({on_c}, x)"))) + list(fol_fc_ask(kb, expr(f"Type({on_c}, y)"))) #type and color of the piece on C

base_of_h = list(list(fol_fc_ask(kb, expr("Base(H, x)")))[0].values())[0] #get the base of H
type_base_of_h = list(fol_fc_ask(kb, expr(f"Type({base_of_h}, x)")))

right_of_c = list(fol_fc_ask(kb, expr("Object_at_right(x, C)")))

top_of_i = list(fol_fc_ask(kb, expr("On(x, I)")))
...
print("Is piece B on top of C", if_b_on_c)
print("The type and color of the piece on C", on_c_color_type)
print("The type of the base of H", type_base_of_h)
print("The pieces on the right of C", right_of_c)
print("The pieces on top of I", top_of_i)
