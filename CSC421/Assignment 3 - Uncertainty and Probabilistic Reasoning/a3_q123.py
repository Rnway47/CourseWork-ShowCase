...
Dnet = BayesNet([ #baynes network for dispena
        ('Visit_Asia', '', 0.01),
        ('Smoker', '', 0.50),
        ('TB', "Visit_Asia", {True: 0.05, False:0.01}),
        ('LC', 'Smoker', {True: 0.1, False: 0.01}),
        ('BC', 'Smoker', {True: 0.60, False: 0.30}),
        ("TB_or_LC", ["LC", "TB"], {(True, True): 1.0, (True, False): 1.0, (False, True): 1.0, (False, False): 0}),
        ("X_ray", "TB_or_LC", {True: 0.98, False: 0.05}),
        ("DP", ["TB_or_LC", "BC"], {(True, True): 0.9, (True, False): 0.7, (False, True): 0.8, (False, False): 0.1})
    ])
...
cpt_l_true = Dnet.variable_node("LC").cpt[True,] #conditional probability table for lung cancer is true
cpt_l_false = Dnet.variable_node("LC").cpt[False,]
...
ea_result = enumeration_ask('DP', dict(Visit_Asia=True, X_ray=True), Dnet)
rs_result_list = (rejection_sampling('DP', dict(Visit_Asia=True, X_ray=True), Dnet, 100000).show_approx()).split()
rs_result = {False:rs_result_list[1][0:-1], True:rs_result_list[3]}

lw_result_list = (likelihood_weighting('DP', dict(Visit_Asia=True, X_ray=True), Dnet, 100000).show_approx()).split()
lw_result = {False:lw_result_list[1][0:-1], True:lw_result_list[3]}

ea_result_true = float(ea_result[True])
rs_result_true = float(rs_result[True])
lw_result_true = float(lw_result[True])
