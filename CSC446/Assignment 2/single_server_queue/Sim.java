...
public static double ServiceTime_sum = 0;
public static double InterarrivalTime_sum = 0;
...
public static void main(String argv[]) {
 ...
 public static void ProcessArrival(Event evt) {
  ...
  InterarrivalTime_sum += exponential(stream, MeanInterArrivalTime);
...
double mean_arrival = InterarrivalTime_sum/TotalCustomers;
double mean_service = ServiceTime_sum/TotalCustomers;
double arrival_rate = 1 / mean_arrival;
double service_rate = 1 / mean_service;
...
//using arrival rate and service rate for server utilization 
// System.out.println( "\tSERVER UTILIZATION                             " + arrival_rate/service_rate);
System.out.println( "\tLONG-RUN AVERAGE TIME SPENT IN SYSTEM PER CUSTOMER                          " + ((TotalBusy+SumResponseTime)/TotalCustomers) + "      MINUTES");
System.out.println( "\tLONG-RUN AVERAGE TIME SPENT IN QUEUE PER CUSTOMER                          " + AVGR + "      MINUTES");
System.out.println( "\tLONG-RUN AVERAGE TIME-AVERAGE NUMBER OF CUSTOMERS IN SYSTEM                         " + arrival_rate * ((TotalBusy+SumResponseTime)/TotalCustomers) + "      CUSTOMERS");
System.out.println( "\tLONG-RUN AVERAGE TIME-AVERAGE NUMBER OF CUSTOMERS IN QUEUE                         " + arrival_rate * AVGR + "      CUSTOMERS");
